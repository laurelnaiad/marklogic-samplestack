...
module.exports = function () {
  ...
  this.Given(
    /clear tag filters/,
    function (next) {
      this.currentPage.filters.clearTags()
          .then(this.notifyOk(next), next);
    }
  );
  ...
  this.When(
    /press key enter in tag search/,
    function (next) {
      this.currentPage.filters.tagSearch.pressEnter()
          .then(this.notifyOk(next), next);
    }
  );

  this.When(
    /filter documents by selecting tag = "(.*)"/,
    function (tag, next) {
      this.currentPage.filters.tagSelect.setValue(tag)
          .then(this.notifyOk(next), next);
    }
  );

  this.When(
    /filter documents by unselecting tag = "(.*)"/,
    function (tag, next) {
      this.currentPage.filters.tagUnselect.setValue(tag)
          .then(this.notifyOk(next), next);
    }
  );
  ...
  this.When(
    /search for tag = "(.*)"/,
    function (tag, next) {
      this.currentPage.filters.tagSearch.setValue(tag)
          .then(this.notifyOk(next), next);
    }
  );
